...
function dashboardResults(node, _ref) {
  ...
  o('.no-results', !values(results).length).text('No results available yet');

  o('.suites.column', suites || 1);

  o('.suites')('.totals', ripple('totals'));

  o('.totals')('.tests', key('tests')).text(String).attr('label', 'Tests');
  o('.totals')('.browsers', key('browsers')).text(String).attr('label', 'Browsers');
  o('.totals')('.passing', key('passing')).text(String).attr('label', 'Passing');

  o('.suites')('.suite', key('suites')).text(key('name'));

  o('.browser.column', values(results)).classed('is-running', key('stats.running'));

  o('.browser')('h1.platform', key('platform'));

  o('.platform')('[version]', []);

  o('.platform')(iconOS, 1).attr('version', key('os.version')).classed('os', 1);

  o('.platform')(iconBrowser, 1).attr('version', key('version'));
  ...
  o('.browser')('.result', proxy(key('suites'), allSuites))('span', 1).text(formatResult).classed('error', by('failures', not(is('0'))));

  o('.result')('i.fails', proxy(key('failures'), str)).classed('error', not(is('0'))).text(String);
  o('.result')('i.delim', '/').text(String);
